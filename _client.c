	...
	for( i = 0; i < BOARD_ROWS; i++ )
	{
		for( j = 0; j < BOARD_COLUMNS; j++ )
		{
			...
			if( canJump( i, j, gamePosition->turn, gamePosition ) ){
				...
				if(!canJumpOver) deleteList(mylinkedlist); //Assignment mentions that jump move have priority over simple. So we don't need simple in that case
...
int alpha_beta(Position *aPosition, char depth, int alpha, int beta, char maximizingPlayer, Move* finalMove){  //recursive minimax function.
	...
	if (moveList == NULL){     //If for any reason, no more moves are available
		deleteList(moveList);
